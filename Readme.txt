...
2.) Open the terminal and navigate to JSON-DYNAMIC and in JSON-DYNAMIC change the directory to the JSON-DYNAMIC using the command:
	cd JSON-DYNAMIC
3.) Enter the command:
	npm install
    This command will download all the dependencies that are required to run the program
4.) Then Enter the Command:
	node server
    This command will run the server at port 8080
